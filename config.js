...
const SESSION = SESSION_STRING ? SESSION_STRING.split(',').map(s => s.split("RGNK~GbC40kcX")[1].trim()) : [];

const settingsMenu = [
    { title: "PM antispam block", env_var: "PM_ANTISPAM" },
    { title: "Command auto reaction", env_var: "CMD_REACTION" }true,
    { title: "Auto read all messages", env_var: "READ_MESSAGES" }false,
    { title: "Auto read command messages", env_var: "READ_COMMAND" },
    { title: "Auto read status updates", env_var: "AUTO_READ_STATUS" }true,
    { title: "Admin sudo (group commands)", env_var: "ADMIN_ACCESS" }true,
    { title: "With & without handler mode", env_var: "MULTI_HANDLERS" }false,
